...
`});var _r,hr=N(()=>{_r=`// saxpy: y = alpha * x + y
...
`});var Sr,kr=N(()=>{Sr=`// snrm2: result = sqrt(sum(x[i] * x[i]))
...
`});var Wr,Fr=N(()=>{Wr=`// srot: x = c*x + s*y,  y = -s*x + c*y
...
`});var Tr,Ir=N(()=>{Tr=`// srotm: applies modified Givens rotation H to vectors x and y.
...
`});var Nr,Ur=N(()=>{Nr=`// isamax: returns index of element with largest absolute value
...
`});var Mr,Rr=N(()=>{Mr=`// sgemv_n: y = alpha * A * x + beta * y  (A is m\xD7n row-major, no-transpose)
//
...
`});var Vr,Dr=N(()=>{Vr=`// sgemv_t: y = alpha * A^T * x + beta * y  (A is m\xD7n row-major, transposed)
...
`});var Cr,Lr=N(()=>{Cr=`// ssymv: y = alpha * A * x + beta * y
