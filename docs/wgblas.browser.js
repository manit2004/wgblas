...
`});var De,Ie=O(()=>{De=`// sdot: result = sum(x[i] * y[i])
...
`});var qe,Ue=O(()=>{qe=`// isamax: returns index of element with largest absolute value
...
`});var Ur,Nt=O(()=>{Ur=`// sgemm_small: C = alpha * op(A) * op(B) + beta * C \u2014 small-tile half of
...
`});var qr,Pt=O(()=>{qr=`// sgemm_large: C = alpha * op(A) * op(B) + beta * C \u2014 large-tile half of
...
`});var Yr,It=O(()=>{Yr=`// sgemmtr_small: C := uplo(alpha * op(A) * op(B) + beta * C) \u2014 small-tile
