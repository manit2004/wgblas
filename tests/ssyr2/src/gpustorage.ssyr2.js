// GPU-resident (GpuMatrix/GpuVector) coverage for ssyr2 — same inputs and
// stdlib oracle as test.ssyr2.js, run through the overload where A/x/y stay
...
// GpuMatrix's own layout wins over ssyr2's layout arg, so a GPU-resident A
// never passes `layout` to ssyr2() itself — only to GpuMatrix.from/unpadMatrix.
...
async function callGpuResident(dev, a) {
  ...
  return withGpuResources(
    ...
    async ({ A, x, y }) => {
      await ssyr2(dev, a.uplo, a.n, a.alpha, x, a.incx, y, a.incy, A, a.lda);
...
test("ssyr2 fixtures (GPU-resident)", async (t) => {
  await runFixtures(
    t,                     // node:test context
    "ssyr2 (GPU-resident)", // routine name — used in the diagnostic label
...
test("ssyr2 edge cases (GPU-resident)", async (t) => {
...
test("ssyr2 edge cases (GPU-resident, column-major)", async (t) => {
