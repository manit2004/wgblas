// GPU-resident (GpuMatrix) coverage for sgemmtr — same inputs and stdlib
// oracle as test.sgemmtr.js, run through the overload where A/B/C stay on
...
import { sgemmtrReference as stdlibReference } from "../../helpers/stdlib.js";
...
async function callGpuResident(dev, a) {
  ...
  return withGpuResources(
    ...
    async ({ A, B, C }) => {
      await sgemmtr(dev, a.uplo, a.transA, a.transB, a.m, a.n, a.k, a.alpha, A, a.lda, B, a.ldb, a.beta, C, a.ldc);
...
test("sgemmtr fixtures (GPU-resident)", async (t) => {
  await runFixtures(
    t,
    "sgemmtr (GPU-resident)",
    device,
    NUM_RUNS,
    THRESHOLD,
    fixtureSpecs,
...
test("sgemmtr edge cases (GPU-resident)", async (t) => {
...
test("sgemmtr edge cases (GPU-resident, column-major)", async (t) => {
