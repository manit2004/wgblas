...
// for a drotm result is 2^-48 rather than 2^-52 — see shaders/f64/dekker.wgsl.
// Same bound shape as srotm's own f32 forwardFactor (tests/srotm/helpers.js),
// resolving h11/h21/h12/h22 from the flag the same way — just with
// double-double's own epsilon.
const eps = 2 ** -48;
...
export function forwardFactor(gpu, ref, a) {
  ...
  if (flag === -1) {
    ...
    throw new Error(`Unknown drotm flag: ${flag}`);
  }
  let maxFactor = 0;
  for (let i = 0; i < a.n; i++) {
