...
export async function isamax(n, x, incx) {
    ...
    if (!_pipelineMain)   _pipelineMain   = await loadShader("isamax");
    if (!_pipelineReduce) _pipelineReduce = await loadShader("reduction/argmax");

    const xBuffer           = xIsGpu ? x._buf : uploadBuffer(x, "isamax-x", false);
    const partialsValBuffer = createStorageBuffer((2 * WGS) * 4, "isamax-partials-val"); //to hold 2*WGS partial max values of f32
    const partialsIdxBuffer = createStorageBuffer((2 * WGS) * 4, "isamax-partials-idx"); //to hold 2*WGS partial max indices of u32
    ...
    const bgMain = createBindGroup(_pipelineMain.getBindGroupLayout(0), [xBuffer, partialsValBuffer, partialsIdxBuffer, paramsBuffer]);
    const { commandEncoder: enc1, ts: ts1 } = runComputePass(_pipelineMain, bgMain, 2 * WGS); //dispatch 2*WGS workgroups

    submit(enc1);

    const bgReduce = createBindGroup(_pipelineReduce.getBindGroupLayout(0), [partialsValBuffer, partialsIdxBuffer, resultBuffer]);
    ...
    if (xIsGpu) {
        destroyBuffers(partialsValBuffer, partialsIdxBuffer, resultBuffer, paramsBuffer, readBuffer);
        if (gpuTime1 !== undefined && gpuTime2 !== undefined) return { index, gpuTimeMs: gpuTime1 + gpuTime2 };
        return { index };
    }

    destroyBuffers(xBuffer, partialsValBuffer, partialsIdxBuffer, resultBuffer, paramsBuffer, readBuffer);
    if (gpuTime1 !== undefined && gpuTime2 !== undefined) return { index, gpuTimeMs: gpuTime1 + gpuTime2 };
    return { index };
}
