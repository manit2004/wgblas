...
export async function isamax(device, n, x, incx) {
    ...
    const bgMain = createBindGroup(pipelineMain.getBindGroupLayout(0), [xBuffer, partialsValBuffer, partialsIdxBuffer, paramsBuffer]);
    const { commandEncoder: enc1, ts: ts1 } = runComputePass(pipelineMain, bgMain, 2 * WGS); //dispatch 2*WGS workgroups

    submit(enc1);

    const bgReduce = createBindGroup(pipelineReduce.getBindGroupLayout(0), [partialsValBuffer, partialsIdxBuffer, resultBuffer]);
