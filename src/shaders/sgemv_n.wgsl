...
//
// One workgroup per output row. Threads stride through A[row, :] and x with
// coalesced reads (consecutive threads → consecutive addresses).
// Four independent accumulators let the GPU pipeline memory requests across
// iterations (ILP=4), hiding the global-memory latency.
