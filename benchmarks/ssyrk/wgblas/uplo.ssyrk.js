// uplo sweep — ssyrk.js pins uplo to its baseline.
//
...
for (const uplo of UPLOS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await ssyrk(device, uplo, "no-transpose", n, n, 1.0, AGpu, lda, 0.0, CGpu, ldc, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await ssyrk(device, uplo, "no-transpose", n, n, 1.0, AGpu, lda, 0.0, CGpu, ldc, "row-major");
...
saveResults("ssyrk", gpuModel, records, { folder: "ssyrk", fileName: "uplo.ssyrk" });
