// trans sweep — ssyrk.js is entirely trans="no-transpose". `trans` flips
// both operand roles at once here (B := A), so unlike sgemm/sgemmtr,
// trans="transpose" is ~1.4-1.5x FASTER.
...
for (const trans of TRANS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await ssyrk(device, "lower", trans, n, k, 1.0, A, k, 0.0, C, n);
    }
