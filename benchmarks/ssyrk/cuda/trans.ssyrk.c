// trans sweep — ssyrk.c is entirely CUBLAS_OP_N. See trans.ssyrk.js for the
// mechanism (trans flips both operand roles at once here since B := A on
// the wgblas side — cuBLAS's own cublasSsyrk has no such internal
// duplication, so this is measured purely for comparison against wgblas).
...
int main(void) {
    ...
    save_results_trans(gpu_model, "ssyrk", "trans.ssyrk", rec_trans, rec_n, med_times, gbs_vals, ri);
    free(rec_trans);
    free(rec_n);
    free(med_times);
    free(gbs_vals);
