// Stride sweep — saxpy.c is entirely incx=incy=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by saxpy.c, not repeated here.
...
int main(void) {
    ...
    save_results_stride(gpu_model, "saxpy", "stride.saxpy", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
