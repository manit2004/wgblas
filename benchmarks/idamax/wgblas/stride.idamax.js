// Stride sweep — idamax.js is entirely incx=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by idamax.js, not repeated here.
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await idamax(device, n, xGpu, stride);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await idamax(device, n, xGpu, stride);
...
saveResults("idamax", gpuModel, records, { folder: "idamax", fileName: "stride.idamax" });
