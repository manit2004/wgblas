// Stride sweep — idamax.c is entirely incx=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by idamax.c, not repeated here.
...
int main(void) {
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasIdamax(handle, n, d_x, stride, &imax_result));
            }
    ...
    save_results_stride(gpu_model, "idamax", "stride.idamax", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
