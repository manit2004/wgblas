...
int main(void) {
    ...
    for (int si = 0; si < num_ns; si++) {
        ...
        for (int i = 0; i < WARMUP_ITERS; i++) {
            CUBLAS_CHECK(cublasIdamax(handle, n, d_x, 1, &imax_result));
        }
        ...
        // throughput: x read only = n * 8 bytes (double)
    ...
    save_results("idamax", gpu_model, bench_ns, med_times, gbs_vals, num_ns);
