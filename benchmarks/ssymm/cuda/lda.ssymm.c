// lda sweep — ssymm.c pins lda to its baseline.
//
// Column byte-stride is lda*4 — cuBLAS is column-major, so lda strides down columns of A.
...
int main(void) {
    ...
    float beta = 0.0f;
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSsymm(handle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, n, n, &alpha, d_A, lda, d_B, ldb, &beta, d_C, ldc));
            }
    ...
    save_results_pad_ex(gpu_model, "ssymm", "lda.ssymm",
