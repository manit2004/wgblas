// side sweep — ssymm.c pins side to its baseline.
//
// Whether A pre- or post-multiplies B — different traversal orders over the same data.
...
int main(void) {
    ...
    int num_v = (int)(sizeof(sides) / sizeof(sides[0]));
    ...
    printf("%-14s  %-10s  %-12s  %-16s  %-12s\n", "side", "n", "compute_ms", "compute_GFLOPs", "compute_GBs");
    printf("%-14s  %-10s  %-12s  %-16s  %-12s\n", "--------------", "----------", "------------", "----------------", "------------");

    for (int vi = 0; vi < num_v; vi++) {
        cublasSideMode_t side = sides[vi];
    ...
    float beta = 0.0f;
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSsymm(handle, side, CUBLAS_FILL_MODE_LOWER, n, n, &alpha, d_A, lda, d_B, ldb, &beta, d_C, ldc));
            }
            ...
            float bytes = (3.0f * n * n + 3.0f * n * n) * sizeof(float);
            float compute_gbs = (bytes / 1e9f) / (med_compute / 1e3f);

            printf("%-14s  %-10d  %-12.4f  %-16.4f  %-12.4f\n", side_names[vi], n, med_compute, compute_gflops, compute_gbs);
            rec_key[ri] = side_names[vi];
            rec_n[ri] = n;
    ...
    save_results_flag(gpu_model, "ssymm", "side.ssymm", "side",
