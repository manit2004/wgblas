// beta sweep — ssymm.js pins beta to its baseline.
//
...
for (const beta of BETAS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await ssymm(device, "left", "lower", n, n, 1.0, AGpu, lda, BGpu, ldb, beta, CGpu, ldc, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await ssymm(device, "left", "lower", n, n, 1.0, AGpu, lda, BGpu, ldb, beta, CGpu, ldc, "row-major");
...
saveResults("ssymm", gpuModel, records, { folder: "ssymm", fileName: "beta.ssymm" });
