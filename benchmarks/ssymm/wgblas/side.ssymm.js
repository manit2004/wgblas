// side sweep — ssymm.js pins side to its baseline.
//
...
for (const side of SIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await ssymm(device, side, "lower", n, n, 1.0, AGpu, lda, BGpu, ldb, 0.0, CGpu, ldc, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await ssymm(device, side, "lower", n, n, 1.0, AGpu, lda, BGpu, ldb, 0.0, CGpu, ldc, "row-major");
...
saveResults("ssymm", gpuModel, records, { folder: "ssymm", fileName: "side.ssymm" });
