// beta sweep — sgemv.js pins beta to its baseline.
//
...
for (const beta of BETAS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await sgemv(device, "no-transpose", n, n, 1.0, AGpu, lda, xGpu, 1, beta, yGpu, 1, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await sgemv(device, "no-transpose", n, n, 1.0, AGpu, lda, xGpu, 1, beta, yGpu, 1, "row-major");
    ...
    // A read + x read + y read/write — logical elements touched, same for every beta
...
saveResults("sgemv", gpuModel, records, { folder: "sgemv", fileName: "beta.sgemv" });
