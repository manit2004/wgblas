...
int main(void) {
    ...
    // write JSON results — m, n, and k all sweep together with `sizes` (a
    // square gemm), so the same array is reused for all three keys.
    record_field fields[] = {
        { "m", FIELD_INT, sizes },
        { "n", FIELD_INT, sizes },
        { "k", FIELD_INT, sizes },
    };
    save_results(gpu_model, "sgemm", "sgemm", fields, 3, med_times, gbs_vals, gflops_vals, num_sizes);
