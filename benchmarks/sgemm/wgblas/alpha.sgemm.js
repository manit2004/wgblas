// alpha sweep — sgemm.js pins alpha to its baseline.
//
...
for (const alpha of ALPHAS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await sgemm(device, "no-transpose", "no-transpose", n, n, n, alpha, AGpu, lda, BGpu, ldb, 0.0, CGpu, ldc, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await sgemm(device, "no-transpose", "no-transpose", n, n, n, alpha, AGpu, lda, BGpu, ldb, 0.0, CGpu, ldc, "row-major");
...
saveResults("sgemm", gpuModel, records, { folder: "sgemm", fileName: "alpha.sgemm" });
