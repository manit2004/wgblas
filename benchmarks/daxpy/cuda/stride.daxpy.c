// Stride sweep — daxpy.c is entirely incx=incy=1 (coalesced, best case).
// Same rationale as saxpy's own stride sweep. stride=1 itself is covered by
// daxpy.c, not repeated here.
...
int main(void) {
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasDaxpy(handle, n, &alpha, d_x, stride, d_y, stride));
            }
    ...
    save_results_stride(gpu_model, "daxpy", "stride.daxpy", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
