...
int main(void) {
    ...
    for (int si = 0; si < num_ns; si++) {
        ...
        for (int i = 0; i < WARMUP_ITERS; i++) {
            CUBLAS_CHECK(cublasSdot(handle, n, d_x, 1, d_y, 1, &dot_result));
        }
        ...
        // throughput: x read + y read = 2 * n * 4 bytes
    ...
    save_results("sdot", gpu_model, bench_ns, med_times, gbs_vals, num_ns);
