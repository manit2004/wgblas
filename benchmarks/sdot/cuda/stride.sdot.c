// Stride sweep — sdot.c is entirely incx=incy=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by sdot.c, not repeated here.
...
int main(void) {
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSdot(handle, n, d_x, stride, d_y, stride, &dot_result));
            }
    ...
    save_results_stride(gpu_model, "sdot", "stride.sdot", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
