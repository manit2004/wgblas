// Stride sweep — sdot.js is entirely incx=incy=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by sdot.js, not repeated here.
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await sdot(device, n, xGpu, stride, yGpu, stride);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await sdot(device, n, xGpu, stride, yGpu, stride);
...
saveResults("sdot", gpuModel, records, { folder: "sdot", fileName: "stride.sdot" });
