// lda sweep — strmm.js pins lda to its baseline.
//
...
for (const pad of PADS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await strmm(device, "left", "lower", "no-transpose", "non-unit", n, n, 1.0, AGpu, lda, BGpu, ldb, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await strmm(device, "left", "lower", "no-transpose", "non-unit", n, n, 1.0, AGpu, lda, BGpu, ldb, "row-major");
...
saveResults("strmm", gpuModel, records, { folder: "strmm", fileName: "lda.strmm" });
