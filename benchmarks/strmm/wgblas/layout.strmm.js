// layout sweep — strmm.js pins layout to its baseline.
//
...
for (const layout of LAYOUTS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await strmm(device, "left", "lower", "no-transpose", "non-unit", n, n, 1.0, AGpu, lda, BGpu, ldb, layout);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await strmm(device, "left", "lower", "no-transpose", "non-unit", n, n, 1.0, AGpu, lda, BGpu, ldb, layout);
...
saveResults("strmm", gpuModel, records, { folder: "strmm", fileName: "layout.strmm" });
