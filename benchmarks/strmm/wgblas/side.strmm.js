// side sweep — strmm.js pins side to its baseline.
//
...
for (const side of SIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await strmm(device, side, "lower", "no-transpose", "non-unit", n, n, 1.0, AGpu, lda, BGpu, ldb, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await strmm(device, side, "lower", "no-transpose", "non-unit", n, n, 1.0, AGpu, lda, BGpu, ldb, "row-major");
...
saveResults("strmm", gpuModel, records, { folder: "strmm", fileName: "side.strmm" });
