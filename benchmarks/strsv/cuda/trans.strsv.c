// trans sweep — strsv.c pins trans to its baseline.
//
// op(A) decides whether the kernel walks A down columns or across rows, so the two
// settings differ in coalescing on identical data.
...
int main(void) {
    ...
    cublasOperation_t transs[] = { CUBLAS_OP_N, CUBLAS_OP_T };
    const char *trans_names[] = { "no-transpose", "transpose" };
    int num_trans = (int)(sizeof(transs) / sizeof(transs[0]));
    ...
    for (int vi = 0; vi < num_trans; vi++) {
        cublasOperation_t trans = transs[vi];
        for (int si = 0; si < num_sizes; si++) {
            ...
            if (bytes_needed > free_mem * 9 / 10) {
                printf("  (skipped trans, n=%d: buffers would exceed available device memory)\n", n);
                continue;
            }
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasStrsv(handle, CUBLAS_FILL_MODE_LOWER, trans, CUBLAS_DIAG_NON_UNIT, n, d_A, lda, d_x, 1));
            }
            ...
            // stored triangle + x — logical elements touched, same for every trans
            float bytes = ((float)n * (n + 1) / 2 + 2.0f * n) * sizeof(float);
            float compute_gbs = (bytes / 1e9f) / (med_compute / 1e3f);

            printf("%-14s  %-10d  %-12.4f  %-12.4f\n", trans_names[vi], n, med_compute, compute_gbs);
            rec_key[ri] = trans_names[vi];
            rec_n[ri] = n;
    ...
    save_results_flag(gpu_model, "strsv", "trans.strsv", "trans",
