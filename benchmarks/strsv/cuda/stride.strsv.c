...
int main(void) {
    ...
    for (int vi = 0; vi < num_stride; vi++) {
        ...
        for (int si = 0; si < num_sizes; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasStrsv(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, n, d_A, lda, d_x, stride));
            }
    ...
    save_results_stride(gpu_model, "strsv", "stride.strsv",
