// uplo sweep — strsv.c pins uplo to its baseline.
//
// Which triangle is referenced. On the wgblas side this is a dispatch-order effect
// (~1.7-1.8x on ssyr/ssyr2); this is the reference point for it.
...
int main(void) {
    ...
    for (int vi = 0; vi < num_uplo; vi++) {
        ...
        for (int si = 0; si < num_sizes; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasStrsv(handle, uplo, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, n, d_A, lda, d_x, 1));
            }
    ...
    save_results_flag(gpu_model, "strsv", "uplo.strsv", "uplo",
