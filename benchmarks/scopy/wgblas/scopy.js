...
const STRIDE = 1; // unit stride — coalesced, best case. See stride.scopy.js for incx/incy > 1.
...
for (const n of SIZES) {
  ...
  const bytes = 2 * n * 4; // x read + y written
  const gbs = bytes / 1e9 / (med / 1e3);
  printRow(COLS, [n, med, gbs]);
  records.push({ n, compute_ms: med, compute_GBs: gbs });
}

saveResults("scopy", gpuModel, records, { folder: "scopy" });
