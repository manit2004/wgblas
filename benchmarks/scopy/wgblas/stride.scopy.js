// Stride sweep — scopy.js is entirely incx=incy=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by scopy.js, not repeated here.
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await scopy(device, n, xGpu, stride, yGpu, stride);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await scopy(device, n, xGpu, stride, yGpu, stride);
    ...
    const bytes = 2 * n * 4; // logical elements touched, same regardless of stride
...
saveResults("scopy", gpuModel, records, { folder: "scopy", fileName: "stride.scopy" });
