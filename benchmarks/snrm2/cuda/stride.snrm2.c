// Stride sweep — snrm2.c is entirely incx=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by snrm2.c, not repeated here.
...
int main(void) {
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSnrm2(handle, n, d_x, stride, &nrm2_result));
            }
    ...
    save_results_stride(gpu_model, "snrm2", "stride.snrm2", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
