// Stride sweep — isamax.js is entirely incx=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by isamax.js, not repeated here.
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await isamax(device, n, xGpu, stride);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await isamax(device, n, xGpu, stride);
...
saveResults("isamax", gpuModel, records, { folder: "isamax", fileName: "stride.isamax" });
