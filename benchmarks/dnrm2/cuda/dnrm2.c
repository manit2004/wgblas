...
#define STRIDE 1 // unit stride — coalesced, best case. See stride.dnrm2.c for incx > 1.
...
int main(void) {
    ...
    for (int si = 0; si < num_ns; si++) {
        ...
        // throughput: x read only, 8 bytes/element
    ...
    save_results_ex("dnrm2", gpu_model, "dnrm2", "dnrm2", bench_ns, med_times, gbs_vals, num_ns);
