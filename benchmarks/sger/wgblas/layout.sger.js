// layout sweep — sger.js pins layout to its baseline.
//
...
for (const layout of LAYOUTS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await sger(device, n, n, 1.0, xGpu, 1, yGpu, 1, AGpu, lda, layout);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await sger(device, n, n, 1.0, xGpu, 1, yGpu, 1, AGpu, lda, layout);
    ...
    // A read + A write + x + y — logical elements touched, same for every layout
...
saveResults("sger", gpuModel, records, { folder: "sger", fileName: "layout.sger" });
