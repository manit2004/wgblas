// stride sweep — sger.js pins stride to its baseline.
//
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await sger(device, n, n, 1.0, xGpu, stride, yGpu, stride, AGpu, lda, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await sger(device, n, n, 1.0, xGpu, stride, yGpu, stride, AGpu, lda, "row-major");
    ...
    // A read + A write + x + y — logical elements touched, same for every stride
    const bytes = (2 * n * n + 2 * n) * 4;
...
saveResults("sger", gpuModel, records, { folder: "sger", fileName: "stride.sger" });
