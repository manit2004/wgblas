// stride sweep — sger.c pins stride to its baseline.
//
...
int main(void) {
    ...
    float alpha = 2.0f;
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSger(handle, n, n, &alpha, d_x, stride, d_y, stride, d_A, lda));
            }
            ...
            // A read + write + x + y — logical elements touched, same for every stride
            float bytes = (2.0f * n * n + 2.0f * n) * sizeof(float);
    ...
    save_results_stride(gpu_model, "sger", "stride.sger",
