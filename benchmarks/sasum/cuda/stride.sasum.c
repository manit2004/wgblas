// Stride sweep — sasum.c is entirely incx=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by sasum.c, not repeated here.
...
int main(void) {
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSasum(handle, n, d_x, stride, &asum_result));
            }
    ...
    save_results_stride(gpu_model, "sasum", "stride.sasum", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
