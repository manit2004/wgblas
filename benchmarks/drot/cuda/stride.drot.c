// Stride sweep — drot.c is entirely incx=incy=1 (coalesced, best case).
// Same rationale as srot's own stride sweep. stride=1 itself is covered by
// drot.c, not repeated here.
...
int main(void) {
    ...
    // Same three magnitudes + odd-neighbour pairing as srot's/dswap's own stride sweeps.
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasDrot(handle, n, d_x, stride, d_y, stride, &c, &s));
            }
    ...
    save_results_stride(gpu_model, "drot", "stride.drot", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
