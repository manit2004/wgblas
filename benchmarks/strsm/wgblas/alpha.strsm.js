// alpha sweep — strsm.js pins alpha to its baseline.
//
...
for (const alpha of ALPHAS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await strsm(device, "left", "lower", "no-transpose", "non-unit", n, n, alpha, AGpu, lda, BGpu, ldb, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await strsm(device, "left", "lower", "no-transpose", "non-unit", n, n, alpha, AGpu, lda, BGpu, ldb, "row-major");
...
saveResults("strsm", gpuModel, records, { folder: "strsm", fileName: "alpha.strsm" });
