// uplo sweep — strsm.js pins uplo to its baseline.
//
...
for (const uplo of UPLOS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await strsm(device, "left", uplo, "no-transpose", "non-unit", n, n, 1.0, AGpu, lda, BGpu, ldb, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await strsm(device, "left", uplo, "no-transpose", "non-unit", n, n, 1.0, AGpu, lda, BGpu, ldb, "row-major");
...
saveResults("strsm", gpuModel, records, { folder: "strsm", fileName: "uplo.strsm" });
