// diag sweep — strsm.js pins diag to its baseline.
//
// A unit diagonal lets the kernel skip the diagonal load, and for the solve the reciprocal
// as well — any difference is exactly that skipped work.
...
const COLS = ["diag", "n", "compute_ms", "compute_GFLOPs", "compute_GBs"];
...
for (const diag of DIAGS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await strsm(device, "left", "lower", "no-transpose", diag, n, n, 1.0, AGpu, lda, BGpu, ldb, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await strsm(device, "left", "lower", "no-transpose", diag, n, n, 1.0, AGpu, lda, BGpu, ldb, "row-major");
...
saveResults("strsm", gpuModel, records, { folder: "strsm", fileName: "diag.strsm" });
