// Stride sweep — drotm.c is entirely incx=incy=1 (coalesced, best case).
// Same rationale as srotm's own stride sweep. stride=1 itself is covered by
// drotm.c, not repeated here.
...
int main(void) {
    ...
    // cublasDrotm with CUBLAS_POINTER_MODE_HOST (default) reads param from host memory
    double param[5] = { -1.0, 0.6, -0.8, 0.8, 0.6 };
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasDrotm(handle, n, d_x, stride, d_y, stride, param));
            }
    ...
    save_results_stride(gpu_model, "drotm", "stride.drotm", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
