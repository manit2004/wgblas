// Stride sweep — dcopy.js is entirely incx=incy=1 (coalesced, best case).
// Same rationale as scopy's own stride sweep. stride=1 itself is covered by
// dcopy.js, not repeated here.
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    // 8 bytes per double-double-emulated element, not 4 — see dcopy.js.
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await dcopy(device, n, xGpu, stride, yGpu, stride);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await dcopy(device, n, xGpu, stride, yGpu, stride);
...
saveResults("dcopy", gpuModel, records, {
  folder: "dcopy",
  fileName: "stride.dcopy",
});
