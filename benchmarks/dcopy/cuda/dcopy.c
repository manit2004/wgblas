...
#define STRIDE 1 // unit stride — coalesced, best case. See stride.dcopy.c for incx/incy > 1.
...
int main(void) {
    ...
    for (int si = 0; si < num_ns; si++) {
        ...
        // throughput: x read + y written = 2 * n * 8 bytes
    ...
    save_results_ex("dcopy", gpu_model, "dcopy", "dcopy", bench_ns, med_times, gbs_vals, num_ns);
