// Stride sweep — dcopy.c is entirely incx=incy=1 (coalesced, best case).
// Same rationale as scopy's own stride sweep. stride=1 itself is covered by
// dcopy.c, not repeated here.
...
int main(void) {
    ...
    // Same three magnitudes + odd-neighbour pairing as scopy's/daxpy's own stride sweeps.
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            size_t bytes_needed = 2ull * (size_t)n * (size_t)stride * sizeof(double);
            ...
            float *h_y_f32 = random_float_array(n * stride, -1.0f, 1.0f);
            double *h_x = malloc((size_t)n * stride * sizeof(double));
            double *h_y = malloc((size_t)n * stride * sizeof(double));
            for (int i = 0; i < n * stride; i++) { h_x[i] = (double)h_x_f32[i]; h_y[i] = (double)h_y_f32[i]; }

            double *d_x, *d_y;
            CUDA_CHECK(cudaMalloc((void **)&d_x, (size_t)n * stride * sizeof(double)));
            CUDA_CHECK(cudaMalloc((void **)&d_y, (size_t)n * stride * sizeof(double)));
            CUDA_CHECK(cudaMemcpy(d_x, h_x, (size_t)n * stride * sizeof(double), cudaMemcpyHostToDevice));
            CUDA_CHECK(cudaMemcpy(d_y, h_y, (size_t)n * stride * sizeof(double), cudaMemcpyHostToDevice));
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasDcopy(handle, n, d_x, stride, d_y, stride));
            }
    ...
    save_results_stride(gpu_model, "dcopy", "stride.dcopy", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
