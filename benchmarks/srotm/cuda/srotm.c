...
#define STRIDE 1 // unit stride — coalesced, best case. See stride.srotm.c for incx/incy > 1.
...
int main(void) {
    ...
    for (int si = 0; si < num_ns; si++) {
        ...
        for (int i = 0; i < WARMUP_ITERS; i++) {
            CUBLAS_CHECK(cublasSrotm(handle, n, d_x, STRIDE, d_y, STRIDE, param));
        }
        ...
        // throughput: x read + x written + y read + y written (param is 5 floats, negligible)
    ...
    save_results_ex("srotm", gpu_model, "srotm", "srotm", bench_ns, med_times, gbs_vals, num_ns);
