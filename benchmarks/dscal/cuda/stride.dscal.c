// Stride sweep — dscal.c is entirely incx=1 (coalesced, best case).
// Same rationale as dasum's own stride.dasum.c. stride=1 itself is covered
// by dscal.c, not repeated here.
...
int main(void) {
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasDscal(handle, n, &alpha, d_x, stride));
            }
    ...
    save_results_stride(gpu_model, "dscal", "stride.dscal", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
