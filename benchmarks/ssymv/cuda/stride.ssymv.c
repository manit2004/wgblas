// stride sweep — ssymv.c pins stride to its baseline.
//
...
int main(void) {
    ...
    float beta = 0.0f;
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSsymv(handle, CUBLAS_FILL_MODE_LOWER, n, &alpha, d_A, lda, d_x, stride, &beta, d_y, stride));
            }
    ...
    save_results_stride(gpu_model, "ssymv", "stride.ssymv",
