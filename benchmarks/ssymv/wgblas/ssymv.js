...
// Max 4096 gives 4096²=16,777,216 total elements ≈ 16.8M (same range as sgemv).
const SIZES = [32, 64, 128, 256, 512, 1024, 1280, 2048, 4096];
const LDA_PAD = 0; // tight lda. See lda.ssymv.js for lda padding effects.
...
for (const size of SIZES) {
  ...
  for (let i = 0; i < BENCH_ITERS; i++) {
    const { gpuTimeMs } = await ssymv(
      device, "lower", n, alpha, AGpu, lda, xGpu, 1, beta, yGpu, 1,
    );
...
saveResults("ssymv", gpuModel, records, { folder: "ssymv" });
