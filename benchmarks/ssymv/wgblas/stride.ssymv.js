// stride sweep — ssymv.js pins stride to its baseline.
//
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await ssymv(device, "lower", n, 1.0, AGpu, lda, xGpu, stride, 0.0, yGpu, stride, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await ssymv(device, "lower", n, 1.0, AGpu, lda, xGpu, stride, 0.0, yGpu, stride, "row-major");
...
saveResults("ssymv", gpuModel, records, { folder: "ssymv", fileName: "stride.ssymv" });
