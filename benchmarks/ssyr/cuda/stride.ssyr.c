// stride sweep — ssyr.c pins stride to its baseline.
//
...
int main(void) {
    ...
    float alpha = 2.0f;
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSsyr(handle, CUBLAS_FILL_MODE_LOWER, n, &alpha, d_x, stride, d_A, lda));
            }
            ...
            float bytes = ((float)n * (n + 1) + n) * sizeof(float);
    ...
    save_results_stride(gpu_model, "ssyr", "stride.ssyr",
