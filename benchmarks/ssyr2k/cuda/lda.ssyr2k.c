// lda sweep — ssyr2k.c is entirely tight lda/ldb. lda and ldb behave
// identically on the wgblas side (see lda.ssyr2k.js), so this sweeps both
// together rather than separately, purely for comparison against wgblas.
    ...
    for (int ti = 0; ti < num_trans; ti++) {
        ...
        for (int pi = 0; pi < num_pads; pi++) {
            ...
            for (int si = 0; si < num_sizes; si++) {
                int n = sizes[si];
                int k = n;
                int lda = k + pad, ldb = k + pad, ldc = n;

                float *h_A = random_float_array((size_t)n * lda, -1.0f, 1.0f);
                float *h_B = random_float_array((size_t)n * ldb, -1.0f, 1.0f);
