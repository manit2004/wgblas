// beta sweep — ssyr2k.js pins beta to its baseline.
//
...
for (const beta of BETAS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await ssyr2k(device, "lower", "no-transpose", n, n, 1.0, AGpu, lda, BGpu, ldb, beta, CGpu, ldc, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await ssyr2k(device, "lower", "no-transpose", n, n, 1.0, AGpu, lda, BGpu, ldb, beta, CGpu, ldc, "row-major");
...
saveResults("ssyr2k", gpuModel, records, { folder: "ssyr2k", fileName: "beta.ssyr2k" });
