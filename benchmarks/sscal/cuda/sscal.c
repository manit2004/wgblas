...
#define STRIDE 1 // unit stride — coalesced, best case. See stride.sscal.c for incx/incy > 1.
...
int main(void) {
    ...
    for (int si = 0; si < num_ns; si++) {
        ...
        // throughput: x read + x written (in-place)
    ...
    save_results_ex("sscal", gpu_model, "sscal", "sscal", bench_ns, med_times, gbs_vals, num_ns);
