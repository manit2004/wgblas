// Stride sweep — sscal.js is entirely incx=1 (coalesced, best case).
...
// plateau) — stride=1 itself is covered by sscal.js, not repeated here.
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await sscal(device, n, alpha, xGpu, stride);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await sscal(device, n, alpha, xGpu, stride);
...
saveResults("sscal", gpuModel, records, { folder: "sscal", fileName: "stride.sscal" });
