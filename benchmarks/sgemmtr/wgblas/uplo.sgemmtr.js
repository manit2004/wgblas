// uplo sweep — sgemmtr.js pins uplo to its baseline.
//
...
for (const uplo of UPLOS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await sgemmtr(device, uplo, "no-transpose", "no-transpose", n, n, n, 1.0, AGpu, lda, BGpu, ldb, 0.0, CGpu, ldc, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await sgemmtr(device, uplo, "no-transpose", "no-transpose", n, n, n, 1.0, AGpu, lda, BGpu, ldb, 0.0, CGpu, ldc, "row-major");
...
saveResults("sgemmtr", gpuModel, records, { folder: "sgemmtr", fileName: "uplo.sgemmtr" });
