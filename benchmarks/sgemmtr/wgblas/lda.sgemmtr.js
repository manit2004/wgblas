// lda sweep — sgemmtr.js pins lda to its baseline.
//
// Row byte-stride is lda*4, so padding moves every row of A relative to the 128-byte
// coalescing boundary.
...
for (const pad of PADS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await sgemmtr(device, "lower", "no-transpose", "no-transpose", n, n, n, 1.0, AGpu, lda, BGpu, ldb, 0.0, CGpu, ldc, "row-major");
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await sgemmtr(device, "lower", "no-transpose", "no-transpose", n, n, n, 1.0, AGpu, lda, BGpu, ldb, 0.0, CGpu, ldc, "row-major");
...
saveResults("sgemmtr", gpuModel, records, { folder: "sgemmtr", fileName: "lda.sgemmtr" });
