// Stride sweep — ddot.c is entirely incx=incy=1 (coalesced, best case).
...
//
// stride=1 itself is covered by ddot.c, not repeated here.
...
int main(void) {
    ...
    for (int ki = 0; ki < num_strides; ki++) {
        ...
        for (int si = 0; si < num_ns; si++) {
            ...
            for (int i = 0; i < n * stride; i++) h_y[i] = (double)h_y_f32[i];
    ...
    save_results_stride(gpu_model, "ddot", "stride.ddot", rec_strides, rec_ns, med_times, gbs_vals, num_recs);
