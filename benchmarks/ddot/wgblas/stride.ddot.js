// Stride sweep — ddot.js is entirely incx=incy=1 (coalesced, best case).
...
//
// stride=1 itself is covered by ddot.js, not repeated here.
...
for (const stride of STRIDES) {
  for (const n of SIZES) {
    ...
    const bytesPerBuffer = n * stride * 4; // hi and lo are separate f32 buffers, so each is n*stride*4
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await ddot(device, n, xGpu, stride, yGpu, stride);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await ddot(device, n, xGpu, stride, yGpu, stride);
    ...
    const bytes = 2 * n * 8; // logical (hi, lo) elements touched, same regardless of stride
...
saveResults("ddot", gpuModel, records, { folder: "ddot", fileName: "stride.ddot" });
