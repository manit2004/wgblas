...
#define STRIDE 1 // unit stride — coalesced, best case. See stride.dswap.c for incx/incy > 1.
...
int main(void) {
    ...
    for (int si = 0; si < num_ns; si++) {
        ...
        // throughput: x read + x written + y read + y written = 4 * n * 8 bytes
    ...
    save_results_ex("dswap", gpu_model, "dswap", "dswap", bench_ns, med_times, gbs_vals, num_ns);
