// uplo sweep — ssyr2.c is entirely CUBLAS_FILL_MODE_LOWER. See uplo.ssyr.js
// for the mechanism — ssyr2 reproduces it almost exactly, ~1.6-1.8x slower
// for upper at n>=1024.
...
int main(void) {
    ...
    for (int ui = 0; ui < num_uplos; ui++) {
        ...
        for (int si = 0; si < num_sizes; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSsyr2(handle, uplo, n, &alpha, d_x, 1, d_y, 1, d_A, lda));
            }
            ...
            float bytes = ((float)(n * (n + 1)) + 2.0f * (float)n) * 4.0f;
    ...
    save_results_uplo(gpu_model, "ssyr2", "uplo.ssyr2", rec_uplo, rec_n, med_times, gbs_vals, ri);
    free(rec_uplo);
    free(rec_n);
    free(med_times);
    free(gbs_vals);
