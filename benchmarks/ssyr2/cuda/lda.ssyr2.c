// lda sweep — ssyr2.c is entirely tight lda (lda = n). See lda.ssyr.js for
// the mechanism — ssyr2 reproduces it almost exactly, ~1.8x slower when
// the row byte-stride (lda*4) isn't a multiple of 128 bytes.
...
int main(void) {
    ...
    save_results_pad(gpu_model, "ssyr2", "lda.ssyr2", rec_pad, rec_n, med_times, gbs_vals, ri);
    free(rec_pad);
    free(rec_n);
    free(med_times);
    free(gbs_vals);
