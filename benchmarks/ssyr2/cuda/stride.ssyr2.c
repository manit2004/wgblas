// stride sweep — ssyr2.c pins stride to its baseline.
//
...
int main(void) {
    ...
    float alpha = 2.0f;
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasSsyr2(handle, CUBLAS_FILL_MODE_LOWER, n, &alpha, d_x, stride, d_y, stride, d_A, lda));
            }
            ...
            float bytes = ((float)n * (n + 1) + 2.0f * n) * sizeof(float);
    ...
    save_results_stride(gpu_model, "ssyr2", "stride.ssyr2",
