// layout sweep — ssyr2.js pins layout to its baseline.
//
...
for (const layout of LAYOUTS) {
  for (const n of SIZES) {
    ...
    for (let i = 0; i < WARMUP_ITERS; i++) {
      await ssyr2(device, "lower", n, 1.0, xGpu, 1, yGpu, 1, AGpu, lda, layout);
    }

    const times = [];
    for (let i = 0; i < BENCH_ITERS; i++) {
      const { gpuTimeMs } = await ssyr2(device, "lower", n, 1.0, xGpu, 1, yGpu, 1, AGpu, lda, layout);
    ...
    // triangle read + write + x + y — logical elements touched, same for every layout
...
saveResults("ssyr2", gpuModel, records, { folder: "ssyr2", fileName: "layout.ssyr2" });
