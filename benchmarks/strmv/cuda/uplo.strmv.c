// uplo sweep — strmv.c pins uplo to its baseline.
//
...
int main(void) {
    ...
    for (int vi = 0; vi < num_uplo; vi++) {
        ...
        for (int si = 0; si < num_sizes; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasStrmv(handle, uplo, CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, n, d_A, lda, d_x, 1));
            }
    ...
    save_results_flag(gpu_model, "strmv", "uplo.strmv", "uplo",
