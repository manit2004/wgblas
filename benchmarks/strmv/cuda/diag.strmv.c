// diag sweep — strmv.c pins diag to its baseline.
//
...
int main(void) {
    ...
    for (int vi = 0; vi < num_diag; vi++) {
        ...
        for (int si = 0; si < num_sizes; si++) {
            ...
            for (int i = 0; i < WARMUP_ITERS; i++) {
                CUBLAS_CHECK(cublasStrmv(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, diag, n, d_A, lda, d_x, 1));
            }
    ...
    save_results_flag(gpu_model, "strmv", "diag.strmv", "diag",
